...
class HSMRDINOHead(DeformableDETRHead):
    ...
    def loss_by_feat_decoder(self,
        ...
        if res_memory is None:
            res_memory = [None for _ in range(len(all_layers_cls_scores))]
            ...
            losses_cls, losses_bbox, losses_iou, losses_hsm = multi_apply(
                self.loss_by_feat_single,
                all_layers_cls_scores,
                all_layers_bbox_preds,
                res_memory,
        ...
        if res_memory[0] is not None:
            loss_dict['loss_hsm'] = losses_hsm[-1]
    ...
    def loss_by_feat(
        ...
        (all_layers_matching_cls_scores, all_layers_matching_bbox_preds,
         all_layers_denoising_cls_scores, all_layers_denoising_bbox_preds, res_memory) = \
            self.split_outputs(
                all_layers_cls_scores, all_layers_bbox_preds, res_memory, dn_meta)

        loss_dict = self.loss_by_feat_decoder(
            all_layers_matching_cls_scores, all_layers_matching_bbox_preds, res_memory,
    ...
    def split_outputs(all_layers_cls_scores: Tensor,
        ...
        if dn_meta is not None:
            ...
            if res_memory is not None:
                res_memory_matching = res_memory[:, :, num_denoising_queries:, :]
            else:
                res_memory_matching = None
        else:
        ...
        return (all_layers_matching_cls_scores, all_layers_matching_bbox_preds,
                all_layers_denoising_cls_scores,
                all_layers_denoising_bbox_preds, res_memory_matching)
